...
export default function UserManagement() {
  ...
  const remove = async (user: User) => {
    if (!window.confirm(`确认删除账号「${user.displayName}」（${user.username}）吗？\n删除后不可恢复，该账号的全部登录会话将立即失效。`)) return
    try { await api.delete(`/system/users/${user.id}`); setMessage('用户已删除'); await load(page) } catch (error) { setMessage(error instanceof Error ? error.message : '删除失败') }
  }
