...
import SampleChoosePreview from '@/pages/SampleChoosePreview'
...
import AiConfigSettings from '@/pages/AiConfigSettings'
...
function Bootstrap() { const init = useAuthStore((state) => state.initialize); useEffect(() => { void init() }, [init]); return <Routes><Route path="/login" element={<Login />} /><Route element={<Protected />}><Route path="/dashboard" element={<Dashboard />} /><Route path="/materials/categories" element={<CategoryTree />} /><Route path="/materials/fabrics" element={<MaterialFabrics />} /><Route path="/partners/providers" element={<RequireRole roles={['admin']}><Providers /></RequireRole>} /><Route path="/partners/customers" element={<RequireRole roles={['admin']}><CustomerInfo /></RequireRole>} /><Route path="/samples/choose" element={<SampleChoose />} /><Route path="/samples/choose-records" element={<SampleRecords />} /><Route path="/info/material-query" element={<MaterialQuery />} /><Route path="/print/labels" element={<LabelPrint />} />
        <Route path="/print/sample-choose/:id" element={<SampleChoosePreview />} /><Route path="/system/users" element={<RequireRole roles={['admin']}><UserManagement /></RequireRole>} /><Route path="/system/roles" element={<RequireRole roles={['admin']}><RolePermissions /></RequireRole>} /><Route path="/system/dictionaries" element={<RequireRole roles={['admin']}><SimpleListPage title="数据字典" description="通过 API 维护数据字典。" endpoint="/dictionaries" fields={fields.dictionaries} /></RequireRole>} />
        <Route path="/system/ai-config" element={<RequireRole roles={['admin']}><AiConfigSettings /></RequireRole>} />
        <Route path="/system/backups" element={<RequireRole roles={['admin']}><BackupManagement /></RequireRole>} />
        <Route path="/system/logs" element={<RequireRole roles={['admin']}><SimpleListPage title="操作日志" description="操作日志来自 API。" endpoint="/system/operation-logs" fields={[{ key: 'action', label: '操作' }, { key: 'resource', label: '资源' }, { key: 'user', label: '操作人', display: (value) => typeof value === 'object' && value ? String((value as { displayName?: string; username?: string }).displayName || (value as { username?: string }).username || '-') : '-' }, { key: 'detail', label: '详情', display: (value) => value ? JSON.stringify(value) : '-' }, { key: 'createdAt', label: '时间', display: (value) => value ? new Date(String(value)).toLocaleString() : '-' }]} readOnly /></RequireRole>} /><Route path="/" element={<Navigate to="/dashboard" replace />} /></Route><Route path="*" element={<Navigate to="/dashboard" replace />} /></Routes> }
