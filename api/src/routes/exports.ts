...
router.post('/sample-chooses/:id', async (req, res, next) => { try {
  ...
  // Logo 文件解析：优先公司信息里的 /uploads 文件；不存在（如容器 uploads 卷尚未执行 seed）时，
  // 回退到镜像内置的 seed-assets/company-logo.png（Dockerfile 已 COPY），保证导出始终带公司 Logo。
  const logoCandidates = [path.resolve(process.cwd(), 'seed-assets', 'company-logo.png')];
  const logoUrl = company?.logoUrl;
  if (logoUrl && logoUrl.startsWith('/uploads/')) logoCandidates.unshift(path.resolve(process.cwd(), logoUrl.slice(1)));
  let logoImageId: number | null = null;
  for (const file of logoCandidates) {
    if (!existsSync(file)) continue;
    logoImageId = await embedImage(workbook, file);
    if (logoImageId !== null) break;
  }
  // 星号线宽度随列宽自适应，铺满表格（老系统为整行星号分隔线）
  const totalColWidth = columns.reduce((sum, col) => sum + col.width, 0);
  const dividerLine = '*'.repeat(Math.max(50, Math.min(220, Math.round((totalColWidth * 7 * 1.35) / 8.6))));
...
router.get('/materials', async (req, res, next) => { try {
  ...
  const admin = req.auth!.role === RoleCode.ADMIN; const workbook = new ExcelJS.Workbook(); const sheet = workbook.addWorksheet('面料资料'); sheet.columns = [{ header: 'Item No.', key: 'itemNo', width: 20 }, { header: '名称', key: 'name', width: 24 }, { header: '类别', key: 'category', width: 18 }, { header: '规格', key: 'specification', width: 26 }, { header: '成分', key: 'composition', width: 22 }, { header: '纱支', key: 'yarnCount', width: 22 }, { header: '密度', key: 'density', width: 12 }, { header: '组织', key: 'construction', width: 18 }, { header: '幅宽', key: 'width', width: 14 }, { header: '克重', key: 'weight', width: 14 }, { header: '颜色', key: 'color', width: 16 }, { header: '色号', key: 'colorNo', width: 16 }, { header: '单位', key: 'unit', width: 10 }, { header: '工厂编码', key: 'factoryNo', width: 16 }, { header: '面料来源', key: 'fabricSource', width: 16 }, { header: '加工方式', key: 'processingMethod', width: 16 }, { header: '产品描述', key: 'productDescription', width: 30 }, { header: '产品备注', key: 'remark', width: 24 }, { header: '状态', key: 'status', width: 10 }, ...(admin ? [{ header: '供应商', key: 'provider', width: 20 }, { header: '成本', key: 'cost', width: 14 }] : []), ...(query.includeImage ? [{ header: '图片', key: 'imageUrl', width: 40 }] : [])]; sheet.getRow(1).font = { bold: true };
  for (const material of materials) { const imageUrl = material.images[0]?.url ?? ''; const row = sheet.addRow({ itemNo: material.itemNo, name: material.name, category: material.category.name, specification: material.specification?.trim() || '/', composition: material.composition ?? '', yarnCount: material.yarnCount ?? '', density: material.density ?? '', construction: material.construction ?? '', width: material.width ?? '', weight: material.weight ?? '', color: material.color ?? '', colorNo: material.colorNo ?? '', unit: material.unit, factoryNo: material.factoryNo ?? '', fabricSource: material.fabricSource ?? '', processingMethod: material.processingMethod ?? '', productDescription: material.productDescription ?? '', remark: material.remark ?? '', status: material.status === RecordStatus.ACTIVE ? '启用' : '停用', provider: admin ? material.provider?.name ?? '' : undefined, cost: admin ? material.cost?.toString() ?? '' : undefined, imageUrl }); if (query.includeImage && imageUrl.startsWith('/uploads/')) { const file = path.resolve(process.cwd(), imageUrl.slice(1)); if (existsSync(file)) { const imageId = await embedImage(workbook, file); if (imageId !== null) { sheet.addImage(imageId, { tl: { col: sheet.columns.length - 1, row: row.number - 1 }, ext: { width: 80, height: 80 } }); row.height = 65; } } } }
