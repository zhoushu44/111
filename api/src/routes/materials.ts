...
const listSchema = z.object({ page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(1).max(100).default(20), keyword: z.string().trim().max(150).optional(), color: z.string().trim().max(100).optional(), status: statusSchema.optional(), categoryId: z.string().uuid().optional(), providerId: z.string().uuid().optional() }).strict();
...
async function recognizeFeatures(base64: string): Promise<{ color: string; texture: string; composition: string; weight: string; width: string; gloss: string }> {
  const body = {
    model: process.env.AI_VISION_MODEL || 'gpt-5.4',
    messages: [{
      role: 'user',
      content: [
  ...
  for (let i = 0; i < 3; i++) {
    try {
      const resp = await fetch(`${process.env.AI_VISION_BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.AI_VISION_API_KEY}` },
