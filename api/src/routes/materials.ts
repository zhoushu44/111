...
import { deleteFromStorage, uploadImagesToStorage } from '../services/storage-config.service.js';
...
const staffSelect = { id: true, itemNo: true, name: true, specification: true, composition: true, construction: true, width: true, weight: true, color: true, colorNo: true, yarnCount: true, density: true, productDescription: true, unit: true, factoryNo: true, fabricSource: true, processingMethod: true, remark: true, labelRemark: true, categoryId: true, status: true, createdAt: true, updatedAt: true, category: { select: { id: true, name: true } }, images: { orderBy: { sortOrder: 'asc' as const }, take: 1 } } as const;
...
router.post('/:id/images', async (req, _res, next) => { try { const { id } = idSchema.parse(req.params); const material = await prisma.materialFabric.findUnique({ where: { id }, select: { status: true } }); if (!material) throw new HttpError(404, '面料不存在'); if (material.status !== RecordStatus.ACTIVE) throw new HttpError(400, '已停用面料不可上传图片'); next(); } catch (error) { next(error); } }, upload.single('image'), async (req, res, next) => { try { const { id } = idSchema.parse(req.params); const body = imageBodySchema.parse(req.body); if (!req.file) throw new HttpError(400, '请选择要上传的图片'); const filename = `${randomUUID()}.webp`; const thumbFilename = `thumb_${filename}`; const webpBuffer = await sharp(req.file.path).rotate().webp({ quality: 85, effort: 4 }).toBuffer(); const thumbBuffer = await sharp(req.file.path).rotate().resize(200, 200, { fit: 'inside', withoutEnlargement: true }).webp({ quality: 75, effort: 4 }).toBuffer(); await unlink(req.file.path).catch(() => undefined); const storedUrls = await uploadImagesToStorage([{ key: `images/${filename}`, body: webpBuffer }, { key: `images/${thumbFilename}`, body: thumbBuffer }]); let url = storedUrls ? storedUrls[0] : `/uploads/materials/${filename}`; let thumbnailUrl = storedUrls ? storedUrls[1] : `/uploads/materials/${thumbFilename}`; if (!storedUrls) { await writeFile(path.join(uploadDirectory, filename), webpBuffer); await writeFile(path.join(uploadDirectory, thumbFilename), thumbBuffer); } const image = await prisma.materialImage.create({ data: { materialId: id, url, thumbnailUrl, sortOrder: body.sortOrder ?? 0 } }); await writeOperationLog({ userId: req.auth!.userId, action: 'UPLOAD_IMAGE', resource: 'MATERIAL_FABRIC', resourceId: id, detail: { imageId: image.id, format: 'webp' }, ip: req.ip }); await recordMaterialChanges({ materialId: id, userId: req.auth!.userId, action: 'UPLOAD_IMAGE', ip: req.ip, diffs: [{ field: 'images', oldVal: '', newVal: '上传了图片' }] }); ok(res, image, '图片已转换为 WebP 并上传成功', 201); } catch (error) { if (req.file?.path) await unlink(req.file.path).catch(() => undefined); next(error); } });
router.delete('/:id/images/:imageId', async (req, res, next) => { try { const { id, imageId } = z.object({ id: z.string().uuid(), imageId: z.string().uuid() }).parse(req.params); const image = await prisma.materialImage.findFirst({ where: { id: imageId, materialId: id } }); if (!image) throw new HttpError(404, '图片不存在'); await prisma.materialImage.delete({ where: { id: image.id } }); await deleteFromStorage(image.url); if (image.thumbnailUrl) await deleteFromStorage(image.thumbnailUrl); if (image.url.startsWith('/uploads/materials/')) await unlink(path.join(uploadDirectory, path.basename(image.url))).catch(() => undefined); if (image.thumbnailUrl && image.thumbnailUrl.startsWith('/uploads/materials/')) await unlink(path.join(uploadDirectory, path.basename(image.thumbnailUrl))).catch(() => undefined); await writeOperationLog({ userId: req.auth!.userId, action: 'DELETE_IMAGE', resource: 'MATERIAL_FABRIC', resourceId: id, detail: { imageId }, ip: req.ip }); await recordMaterialChanges({ materialId: id, userId: req.auth!.userId, action: 'DELETE_IMAGE', ip: req.ip, diffs: [{ field: 'images', oldVal: '删除了图片', newVal: '' }] }); ok(res, null, '图片已删除'); } catch (error) { next(error); } });
// 公司颜色色子表：CRUD
...
router.post('/:id/colors/:colorId/image', async (req, _res, next) => {
  ...
  try {
    ...
    const filename = `${randomUUID()}.webp`; const webpBuffer = await sharp(req.file.path).rotate().webp({ quality: 85, effort: 4 }).toBuffer(); await unlink(req.file.path).catch(() => undefined); const storedUrls = await uploadImagesToStorage([{ key: `images/${filename}`, body: webpBuffer }]); let colorUrl = storedUrls ? storedUrls[0] : `/uploads/materials/${filename}`; if (!storedUrls) await writeFile(path.join(uploadDirectory, filename), webpBuffer); const color = await prisma.materialColor.findFirst({ where: { id: colorId, materialId: id } }); if (color?.image) await deleteFromStorage(color.image); if (color?.image && color.image.startsWith('/uploads/materials/')) await unlink(path.join(uploadDirectory, path.basename(color.image))).catch(() => undefined); const updated = await prisma.materialColor.update({ where: { id: colorId }, data: { image: colorUrl } });
...
router.delete('/:id/colors/:colorId/image', async (req, res, next) => {
  try {
    ...
    if (color.image) await deleteFromStorage(color.image);
