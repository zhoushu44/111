...
function routes(prefix: '/providers' | '/customers', resource: 'PROVIDER' | 'CUSTOMER') {
  ...
  router.get(prefix, async (req, res, next) => { try { const query = listSchema.parse(req.query); const where = whereFor(query, isProvider); const [list, total] = isProvider ? await Promise.all([prisma.provider.findMany({ where, orderBy: [{ name: 'asc' }, { code: 'asc' }], skip: (query.page - 1) * query.pageSize, take: query.pageSize }), prisma.provider.count({ where })]) : await Promise.all([prisma.customer.findMany({ where, orderBy: [{ name: 'asc' }, { code: 'asc' }], skip: (query.page - 1) * query.pageSize, take: query.pageSize }), prisma.customer.count({ where })]); ok(res, { list, total, page: query.page, pageSize: query.pageSize }); } catch (error) { next(error); } });
