...
export const errorHandler: ErrorRequestHandler = (error, _req, res, _next: NextFunction) => {
  ...
  if (error instanceof ZodError) {
    const summary = error.issues.slice(0, 3).map((issue) => issue.message).filter(Boolean).join('；');
    return res.status(400).json({ code: 400, message: summary ? `请求参数无效：${summary}` : '请求参数无效', data: error.flatten() });
  }
