...
11. **标签打印与扫码**：70mm×40mm 标签预览；Argox CP-2140M/3140 浏览器打印适配；二维码存储 Item No.；临时备注可覆盖或追加且不回写面料资料。打印可靠性须与老系统 HSTIP 对齐：份数由打印指令（ZPL `^PQ` / TSPL `SET COUNT`）显式控制、方向由指令显式固定（`^FWI`/旋转参数），不依赖打印机驱动默认份数与方向；标签 Remark 为标签专用字段，过滤价格类内容（如 USD/CNY 报价），不直接透传含价格的原始备注。
...
| 标签打印 | 单个/批量预览、份数、临时备注 | 尺寸 70mm×40mm；二维码内容为 Item No.；份数与方向由打印指令显式固定，实际打印份数=系统输入份数；Remark 不显示价格（过滤 USD/CNY 报价类内容） |
...
7. 标签按 70mm×40mm 预览和打印，二维码可由扫码枪识别为 Item No.，临时备注不回写主数据；输入 N 份实际只打出 N 份（打印机驱动默认份数/张数补偿不影响结果），文字方向正确不旋转；标签 Remark 一栏不出现 USD/CNY 等价格内容；Argox 打印机完成驱动物理实测（纸张 70×40mm、Gap 校准、方向 0°）。
